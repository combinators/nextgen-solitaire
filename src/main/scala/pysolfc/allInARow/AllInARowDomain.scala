...
/**
  ...
  */
class AllInARowDomain(override val solitaire:Solitaire) extends SolitaireDomain(solitaire) with GameTemplate with PythonSemanticTypes {

  /**
    * Convert ID into string. Each different variation adds a unique ID to the pygames grouping
   */
  @combinator object allInARowID extends IdForGame(pygames.allInARow)

  @combinator object OutputFile {
    def apply: String = "allInARow"
