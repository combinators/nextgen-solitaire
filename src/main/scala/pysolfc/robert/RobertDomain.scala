...
/**
  ...
  */
class RobertDomain(override val solitaire:Solitaire) extends SolitaireDomain(solitaire) with GameTemplate with PythonSemanticTypes {

  /**
    * Convert ID into string. Each different variation adds a unique ID to the pygames grouping
   */
  @combinator object robertID extends IdForGame(pygames.robert)

  @combinator object OutputFile {
    def apply: String = "robert"
