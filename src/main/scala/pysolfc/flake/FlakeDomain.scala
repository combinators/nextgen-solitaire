...
/**
  ...
  */
class FlakeDomain(override val solitaire:Solitaire) extends SolitaireDomain(solitaire) with GameTemplate with PythonSemanticTypes {

  /**
    * Convert ID into string. Each different variation adds a unique ID to the pygames grouping
   */
  @combinator object flakeID extends IdForGame(pygames.flake)

  @combinator object OutputFile {
    def apply: String = "flake"
