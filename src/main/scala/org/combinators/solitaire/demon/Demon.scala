...
/** Loads and runs the combinators to generate the Archway variation.
  ...
  * @param webJars
  */
class Demon @Inject()(webJars: WebJarsUtil, applicationLifecycle: ApplicationLifecycle) extends InhabitationController(webJars, applicationLifecycle) with RoutingEntries {

  val solitaire = new domain.demon.Domain()

  lazy val repository = new DemonDomain(solitaire) with controllers {}
